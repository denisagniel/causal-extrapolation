...
\begin{assumption}[Limited between-state ATT heterogeneity]\label{p-to-p-plus-one-heterogeneity}
\[
\E_{\P_{p+1}}\{Y_{i,p+1}(1) - Y_{i,p+1}(0) \mid A_{ip} = 1\} = \E_{\P_{p+1}}\{Y_{i,p+1}(1) - Y_{i,p+1}(0) \mid A_{i,p+1} = 1\}.
\]
\end{assumption}
...
\begin{assumption}[Strict within-group ATT time homogeneity]
    $\theta_{gt} = \theta_{gs} \equiv \theta_{g\cdot}$, for all $s, t \in \{1, ..., p+1\}$.\label{within-group-strict-time-homogeneity}
\end{assumption}
\begin{assumption}[Limited within-group between-state ATT heterogeneity]\label{within-group-p-to-p-plus-one-heterogeneity}
\[
\E_{\P_{p+1}}\{Y_{i,p+1}(1) - Y_{i,p+1}(0) \mid A_{ip} = 1, G_i = g\} = \E_{\P_{p+1}}\{Y_{i,p+1}(1) - Y_{i,p+1}(0) \mid A_{i,p+1} = 1, G_i = g\}.
\]
\end{assumption}
...
Suppose the group-time ATTs $\theta_{gt}$ are identified from the observed data under the researcher's design for $t \leq p$, $g \in \cG$; that $\theta_{gt} = f(g,t;\bgamma)$ with $\bgamma$ uniquely determined by $(\theta_{gt})_{g,t,\, t \leq p}$; and that within-group limited between-state heterogeneity (Assumption~\ref{within-group-p-to-p-plus-one-heterogeneity}) holds. Then the FATT $\theta_{p+1}$ is identified by $\theta_{p+1} = \sum_{g}\P(G_i = g | A_{ip} = 1) f(g, p+1; \bgamma)$.
\end{proposition}
...
Estimation under Path~3 has two ingredients, both off-the-shelf: an estimator $\widehat{\tau}(\cdot)$ of the conditional ATT and a target covariate sample. The FATT estimator integrates the first over the second,
\begin{align}
    \widehat{\theta}_{p+1} = \frac{1}{n^*}\sum_{i=1}^{n^*} \widehat{\tau}(\bX_i^*), \label{eq:fatt-path3-est}
\end{align}
where $\{\bX_i^*\}_{i=1}^{n^*}$ is a sample from the target distribution $F_{\bX | A_{ip}=1}^{p+1}$ (the treated units' covariates at $p+1$; the study sample's treated units when covariates are time-invariant, or forecasted/simulated draws otherwise). In the parametric special case $\widehat{\tau}(x) = m(x;\widehat{\bbeta})$ this is $n^{*-1}\sum_i m(\bX_i^*;\widehat{\bbeta})$; with a nonparametric learner $\widehat{\tau}$ (e.g.\ a causal forest) it is the average of the predicted conditional effects over the target sample.

\paragraph{Path 3 EIF.} Because $\widehat{\tau}(\cdot)$ is in general not $\sqrt{n}$-consistent (with continuous $\bX$ it converges at a slower nonparametric rate), we do not propagate an influence function for $\widehat{\tau}$ itself. Instead, the scalar FATT $\theta_{p+1} = \E_{\mathrm{src}}[w(\bX)\tau(\bX)]$ is a pathwise-differentiable functional admitting a \emph{Neyman-orthogonal} estimating equation, and it is this functional that is $\sqrt{n}$-estimable. For the unconfoundedness design, with outcome regressions $\mu_a(x)$, propensity $e(x)$, and density ratio $w(x) = dF_{\bX | A_{ip}=1}^{p+1}/dF_{\bX}^{\mathrm{src}}(x)$, the efficient influence function is the doubly-robust transport score
\begin{align}
    \phi_{\psi_3}(O_i) = w(\bX_i)\big(\tau(\bX_i) - \theta_{p+1}\big) + w(\bX_i)\left[\frac{A_i\big(Y_i - \mu_1(\bX_i)\big)}{e(\bX_i)} - \frac{(1-A_i)\big(Y_i - \mu_0(\bX_i)\big)}{1-e(\bX_i)}\right], \label{eq:eif-path3}
\end{align}
plus a correction term when $w$ is itself estimated from a finite target sample; the conditional parallel-trends design replaces the bracketed augmentation with the corresponding doubly-robust difference-in-differences score, reweighted by $w$. The derivation is in Appendix~\ref{app:eif3}. Because the score in \eqref{eq:eif-path3} is orthogonal to the nuisances $(\mu_a, e, w)$, plugging in cross-fitted nonparametric or machine-learning estimates of these nuisances at the usual $o_\P(n^{-1/4})$ rate yields an asymptotically linear, $\sqrt{n}$-consistent $\widehat{\theta}_{p+1}$ with influence function \eqref{eq:eif-path3} \citep{chernozhukovDoubleDebiasedMachine2018}.

This is the same machinery used for Paths~1 and~2, with one structural difference: there the first-stage \emph{object} $\widehat{\theta}_{gt}$ is itself $\sqrt{n}$ and the map to $\theta_{p+1}$ is a finite-dimensional delta method, whereas here the first-stage \emph{function} $\widehat{\tau}$ is estimated slowly and $\sqrt{n}$ inference for the scalar $\theta_{p+1}$ is recovered through orthogonality. We note this so that the asymmetry is not mistaken for an inconsistency. If the conditional model is misspecified (e.g.\ a parametric $m$ that omits relevant heterogeneity), $\widehat{\theta}_{p+1}$ converges to a pseudo-true value rather than the FATT; researchers should assess conditional-model fit and the plausibility of the overlap condition (bounded $w$), discussed next.
...
For Path 3, the nuisance functions---the conditional outcome means $\mu_a(x)$ and propensity $e(x)$ (unconfoundedness design), or their difference-in-differences analogs, together with the density ratio $w(x)$---are estimated by cross-fitting, with $L_2$ convergence rates whose products are $o_{\P}(n^{-1/4})$. The conditional effect $\widehat{\tau}(\cdot)$ need not be $\sqrt{n}$-consistent.
\end{assumption}

\begin{assumption}[Overlap for transport (Path 3)]\label{RC9}
The density ratio $w(x) = dF_{\bX | A_{ip}=1}^{p+1}/dF_{\bX}^{\mathrm{src}}(x)$ exists and is bounded, $\|w\|_\infty < \infty$ (or at least $\E_{\mathrm{src}}[w(\bX)^2] < \infty$); equivalently, the future target covariate support is contained in the support of the source distribution on which $\tau$ is estimated.
\end{assumption}

\begin{assumption}[Target distribution access (Path 3)]\label{RC10}
Either (a) the target sample $\{\bX_i^*\}_{i=1}^{n^*}$ is representative of $F_{\bX | A_{ip}=1}^{p+1}$ (e.g., drawn i.i.d. from the target distribution), or (b) a sampler faithfully generates draws from it. When the target is the study's treated units under time-invariant covariates, this holds by construction.
\end{assumption}

\begin{remark}
Assumptions \ref{RC1}--\ref{RC3} and \ref{RC6} are standard for semiparametric inference with first-stage estimation. Assumptions \ref{RC4}--\ref{RC5} are specific to Path~2 (parametric extrapolation). Assumptions \ref{RC8}--\ref{RC10} are specific to Path~3 (covariate transport): \ref{RC8} gives the doubly-robust product-rate conditions, \ref{RC9} the overlap/bounded-density-ratio condition required for transport, and \ref{RC10} access to the target covariate distribution. Assumption \ref{RC7} relaxes Donsker when using modern machine-learning methods via cross-fitting, following \citet{chernozhukovDoubleDebiasedMachine2018}.
\end{remark}
...
\textit{Setup:} Under Path~3 the target is the transported conditional effect
\[
\theta_{p+1} = \E_{\mathrm{src}}\!\left[w(\bX)\,\tau(\bX)\right], \qquad w(x) = \frac{dF_{\bX | A_{ip}=1}^{p+1}}{dF_{\bX}^{\mathrm{src}}}(x),
\]
where $\tau(x) = \mu_1(x) - \mu_0(x)$ under the unconfoundedness design, with $\mu_a(x) = \E[Y \mid \bX = x, A = a]$ and propensity $e(x) = \P(A=1 \mid \bX = x)$. Unlike Paths~1--2, $\theta_{p+1}$ is not a finite-dimensional smooth function of the marginal $\theta_{gt}$; it is a functional of the nuisances $(\mu_0, \mu_1, e, w)$, and the conditional effect $\tau$ is estimated nonparametrically. We therefore derive the influence function directly rather than by a delta method through $\bbeta$.

\textit{Derivation.} The functional $\psi(P) = \E_P[w \tau]$ is pathwise differentiable; its efficient influence function combines (i) the variation of the covariate average of $\tau$ and (ii) the standard augmentation that makes the score orthogonal to the outcome and propensity nuisances. Taking a parametric submodel and differentiating, the term from averaging $\tau$ over the target gives $w(\bX)(\tau(\bX) - \psi)$, and the term from estimating $\tau = \mu_1 - \mu_0$ gives the inverse-propensity-weighted residual augmentation, also reweighted by $w$. Collecting,
\[
\phi_{\psi_3}(O) = w(\bX)\big(\tau(\bX) - \theta_{p+1}\big) + w(\bX)\left[\frac{A\big(Y - \mu_1(\bX)\big)}{e(\bX)} - \frac{(1-A)\big(Y - \mu_0(\bX)\big)}{1 - e(\bX)}\right] + r(O), \tag{$\ast$}
\]
where $r(O)$ is the contribution from estimating the density ratio $w$ when the target distribution is itself estimated from a finite sample (and $r \equiv 0$ when the target covariates are treated as fixed/known).

\textit{Difference-in-differences design.} Under conditional parallel trends the bracketed augmentation in $(\ast)$ is replaced by the conditional doubly-robust DiD score of \citet{santannaDoublyRobustDifferenceindifferences2020}, reweighted by $w$. The key observation is that \emph{conditional parallel trends is conditional unconfoundedness applied to the outcome change} $\Delta Y_i = Y_{i,\mathrm{post}} - Y_{i,\mathrm{pre}}$: parallel trends states $\E[\Delta Y_i(0) \mid \bX_i, A_i = 1] = \E[\Delta Y_i(0) \mid \bX_i, A_i = 0]$, i.e.\ the untreated potential change is mean-independent of treatment given $\bX_i$. The entire unconfoundedness derivation therefore transfers under the substitution
\[
Y \;\longmapsto\; \Delta Y, \qquad \mu_a(x) \;\longmapsto\; m_a(x) := \E[\Delta Y \mid \bX = x, A = a],
\]
with the conditional ATT recovered as $\tau(x) = m_1(x) - m_0(x)$, equivalently the DR-DiD structural constraint $m_1(x) = m_0(x) + \tau(x)$. Substituting into the augmentation bracket of $(\ast)$ gives the DR-DiD transport influence function
\[
\phi_{\psi_3}^{\mathrm{DiD}}(O) = w(\bX)\big(\tau(\bX) - \theta_{p+1}\big) + w(\bX)\left[\frac{A\big(\Delta Y - m_1(\bX)\big)}{e(\bX)} - \frac{(1-A)\big(\Delta Y - m_0(\bX)\big)}{1 - e(\bX)}\right] + r(O), \tag{$\ast\ast$}
\]
where $m_1 = m_0 + \tau$, so $m_0$ is the only outcome-change nuisance that must be estimated (the treated-arm regression is pinned by the conditional effect $\tau$, which is supplied as the first-stage input). Because parallel trends removes the level confounding by first-differencing, no separate $\mu_a$ level regression is needed; the nuisances entering $(\ast\ast)$ are $(m_0, e, w)$, with $\tau$ treated as the fixed first-stage CATE. The bracket in $(\ast\ast)$ is exactly the correction implemented by \texttt{score\_drdid} in the companion package. Orthogonality (to $m_0$, $e$, and $w$, with $\tau$ held fixed) and the collapse property below are inherited from the unconfoundedness case via the $Y \mapsto \Delta Y$, $\mu_a \mapsto m_a$ substitution.

\textit{Overlap.} Both scores divide by $e(\bX)$ and $1 - e(\bX)$, so we require strict overlap: there is $\eta > 0$ with $e(\bX) \in (\eta, 1-\eta)$ almost surely. Together with the bounded density ratio of Assumption~\ref{assump:target-dist} ($\|w\|_\infty < \infty$), this keeps the score's variance finite. In practice weak overlap---$e$ near $0$ or $1$, or a large $w$---is the binding limitation: it inflates the variance and can make the effective sample size collapse, as we find in the application.

\textit{Orthogonality and rates.} The score $(\ast)$ is Neyman-orthogonal: its Gateaux derivative with respect to each of its nuisances---$(\mu_0, \mu_1, e, w)$ under unconfoundedness, or $(m_0, e, w)$ with $\tau$ fixed under the difference-in-differences form $(\ast\ast)$---vanishes at the truth. Consequently, estimating these nuisances by cross-fitted nonparametric or machine-learning methods at $L_2$ rate $o_\P(n^{-1/4})$ leaves the resulting product bias at $o_\P(n^{-1/2})$, so the one-step/estimating-equation estimator $\widehat\theta_{p+1}$ is asymptotically linear with influence function $(\ast)$ (or $(\ast\ast)$), and $\sqrt{n}(\widehat\theta_{p+1} - \theta_{p+1}) \leadsto N(0, \E[\phi_{\psi_3}^2])$ \citep{chernozhukovDoubleDebiasedMachine2018}. We do \emph{not} require $\widehat\tau(\cdot)$ to be $\sqrt{n}$-consistent; the $\sqrt{n}$ rate is recovered at the level of the scalar functional through orthogonality.

\begin{lemma}[Collapse to the ATT influence function]\label{lem:path3-collapse}
If the target covariate distribution equals the source distribution, $F_{\bX | A_{ip}=1}^{p+1} = F_{\bX}^{\mathrm{src}}$, then $w \equiv 1$, $r \equiv 0$, and $(\ast)$ reduces to
\[
\phi_{\psi_3}(O) = \big(\tau(\bX) - \theta_{p+1}\big) + \frac{A\big(Y - \mu_1(\bX)\big)}{e(\bX)} - \frac{(1-A)\big(Y - \mu_0(\bX)\big)}{1 - e(\bX)},
\]
which is exactly the efficient influence function of the (augmented inverse-propensity-weighted) average treatment effect. Thus Path~3 nests the no-covariate-shift case as the ordinary doubly-robust ATT estimator. The identical collapse holds for the difference-in-differences score $(\ast\ast)$ under the $Y \mapsto \Delta Y$, $\mu_a \mapsto m_a$ substitution: when $w \equiv 1$ and $r \equiv 0$ it reduces to the ordinary conditional DR-DiD efficient influence function \citep{santannaDoublyRobustDifferenceindifferences2020}.
\end{lemma}

Lemma~\ref{lem:path3-collapse} is a useful correctness check: when the target and source covariate distributions coincide, the Path~3 influence function reproduces the standard doubly-robust ATT (or DR-DiD) efficient influence function, and the companion package unit-tests exactly this collapse for both designs. One caveat on the DiD comparison: the transport estimand weights the treated target by the hard indicator $\mathbf{1}\{A_{ip}=1\}$, whereas \citet{santannaDoublyRobustDifferenceindifferences2020} weight their ATT by the smooth propensity odds $e/\E[A]$. The two agree in the point-identified estimand but not in their influence functions, so the collapse matches the DR-DiD \emph{point} EIF; standard errors need not equal those from the Sant'Anna--Zhao ATT estimator.
...
\textbf{Step 2 (Path 3):} Here the first-stage object is the conditional effect $\tau(\cdot)$ (not the finite-dimensional $\theta_{gt}$), and $\widehat{\tau}$ need not be $\sqrt{n}$-consistent. Instead $\widehat{\theta}_{p+1}$ solves the estimating equation built from the orthogonal score $(\ast)$ of Appendix~\ref{app:eif3}. By Neyman orthogonality of $(\ast)$ with respect to the nuisances $(\mu_0,\mu_1,e,w)$ and the product-rate condition of Assumption~\ref{RC8} ($o_\P(n^{-1/4})$ per nuisance, via cross-fitting), the plug-in bias from nuisance estimation is $o_\P(n^{-1/2})$, so
\[
\sqrt{n}(\widehat{\theta}_{p+1} - \theta_{p+1}) = n^{-1/2}\sum_i \phi_{\psi_3,i} + o_{\P}(1),
\]
with $\phi_{\psi_3,i}$ from Appendix~\ref{app:eif3}. Unlike Paths~1--2 this is not a finite-dimensional delta method; $\sqrt{n}$ inference for the scalar $\theta_{p+1}$ is recovered at the functional level through orthogonality.
