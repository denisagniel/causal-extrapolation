...
This basic possibility of dynamic effects has driven a great deal of recent advances in methods for causal inference with panel data \citep{sunEstimatingDynamicTreatment2021,wangAdvancesDifferenceindifferencesMethods2024a}. Recent surveys clarify how heterogeneity and dynamics enter identification and estimation in this setting \citep{arkhangelskyCausalModelsLongitudinal2024,dechaisemartinTwowayFixedEffects2023}. It is now standard to assume that causal effects vary both with time of treatment and time since implementation. These effects (often called ``group-time'' effects) may be of interest in themselves but more typically are aggregated into coarser effects like an ``overall'' average treatment effect on the treated (ATT). Assuming such dynamic effects solves a problem with classical methods like two-way fixed effects: they may incur bias in the presence of time-heterogeneous effects because they implicitly include comparisons between units treated at different times in their estimate of the ATT \citep{sunEstimatingDynamicTreatment2021}.

This is a genuine methodological advance, and it comes at a hidden cost. The group-time ATT, the overall ATT, and any weighted aggregate of them are all defined on the distribution of the data at some \emph{observed} period: each is a statement about what already happened. The decisions that motivate estimating them in the first place are not. A state legislature asking whether to keep a law it passed, or to pass one it has not, next session is asking about a period the study does not cover. If effects can drift arbitrarily with time -- which is exactly the possibility the group-time literature takes seriously -- then no amount of care about the observed window licenses using an estimate from that window to answer a question about a period outside it. The very assumption that motivated disaggregating the ATT into group-time effects has, without being noticed, undercut the field's warrant for using any of those effects, or their aggregate, to inform a current or future decision.

The gap this leaves is not an estimation gap but an \emph{estimand} gap, and it stays invisible for exactly as long as the target is never written down for a period outside the sample. Applied practice closes it informally: an ATT estimated on historical adopters gets discussed as though it were the effect of the policy now. That move is an assumption. It is usually left unstated, and it can be wrong in a direction that matters. What is missing is a family of estimands located at a genuinely future period, together with a statement of exactly which invariance assumption buys each one from historical data, and inference that carries the first-stage uncertainty through the extrapolation rather than discarding it.

The question of when and how historical evidence can inform current or future decisions is central to the literature on generalizing and transporting causal effects \citep{coleGeneralizingEvidenceRandomized2010,tiptonImprovingGeneralizationsExperiments2013,manskiPublicPolicyUncertain2013}. If one estimates the effect of a public policy to be beneficial (e.g., improves life expectancy) among states that implemented it in historical data, does that mean that it would be wise to maintain that policy in places that have already implemented it, or to implement it in places that have not? If effects are unrestrictedly dynamic, it is impossible to say.

This paper's organizing move is to insist that the researcher choose \emph{what is held invariant} across the transition from the observed window to the future period, rather than leave that choice implicit. Three choices are available: the level of the effect itself, the shape of its time path, or the covariate-conditional effect function. Each choice is a different extrapolation function plugged into the same forward map from the observed group-time effects to a future estimand, and each carries a correspondingly different failure mode. Taking dynamics \emph{less} seriously (Path~1) holds the effect level invariant: some form of time homogeneity identifies the future effect from historical data directly, and we argue that an informal version of this is already standard practice, where average treatment effects estimated on historical data are treated as if they are stable enough over time to inform current debates \citep{gablerDealingHeterogeneityTreatment2009,cintronHeterogeneousTreatmentEffects2022}. The notion of policy-relevant treatment effects formalizes which estimands answer counterfactual policy questions \citep{heckmanPolicyRelevantTreatmentEffects2001,sasakiEstimationInferencePolicy2023}. Taking dynamics \emph{more} seriously trades that constancy for an explicit model of change, and splits into two further choices of what to hold fixed. Path~2 holds the \emph{shape} of the time path invariant: researchers specify temporal dynamics (e.g., trends in event time) and extrapolate the fitted function to the future period \citep{khosrowiExtrapolationCausalEffects2019,maebaExtrapolationTreatmentEffect2024}. Path~3 holds the \emph{covariate-conditional effect function} invariant instead of anything about time directly: researchers model treatment-effect heterogeneity via covariates and integrate the resulting conditional effect over the future covariate distribution. Path~3 is robust to regime change precisely because it never assumes temporal patterns continue -- if treatment effects are driven by observable covariates that capture ``deep parameters'' \citep{lucasjrEconometricPolicyEvaluation1976}, reduced-form temporal patterns may break while the structural, covariate-conditional relationship remains predictive. This connects to structural methods that enable extrapolation across regimes \citep{heckmanStructuralEquationsTreatment2005,pearlExternalValidityCalculus2022}.

Across fields, three broad strategies recur for making causal claims about a period the data do not cover: structural or parameter invariance (e.g., policy-invariant deep parameters), statistical transport or reweighting of effects across populations, and explicit modeling of how effects change. Paths~1--3 map onto these traditions in turn, with Path~3 most directly invoking the Lucas critique's emphasis on structural parameters. We call the estimands these three paths identify \emph{future causal effects}, to make explicit that they are located outside the time range of the observed study rather than being redescriptions of anything already estimable from it.

Our contribution is threefold. First, we define the future causal effect family -- the future ATT, ATU, and ATE, corresponding one-to-one to the repeal, adopt, and universal-adoption decisions available to a policymaker -- and show all three share one identification structure, the future conditional effect averaged over a different target covariate distribution (Section~\ref{sec:future-effect-family}). Second, we supply the three identification routes above, stated as three different invariance assumptions plugged into that same structure (Sections~\ref{sec:effect-level-invariance}--\ref{sec:covariate-transport}), together with an efficient influence function for each so that standard errors and confidence intervals propagate from whatever first-stage estimator the researcher already uses -- by linearity for the homogeneity and parametric routes, and by a Neyman-orthogonal doubly-robust transport score for the covariate route, whose estimating object is generally not root-$n$ estimable on its own (Section~\ref{sec:inference}). Third, for the parametric route we give what is, to our knowledge, the first systematic framework for temporal-model selection in causal extrapolation: a time-series cross-validation criterion that scores candidate models on extrapolation error and interval coverage rather than in-sample fit (Section~\ref{sec:model-selection}). We support these results with simulation evidence isolating the aggregation-and-extrapolation step from any particular first-stage design (Section~\ref{sec:simulations}), and with an application to a staggered state-level policy with a genuine out-of-sample validation window, where all three routes fail in the same direction (Section~\ref{sec:application}) -- a result we return to in the discussion (Section~\ref{sec:discussion}) alongside the paper's stated scope limitations.
...
\section{The Future Effect Family and Its Identification Problem}\label{sec:future-effect-family}

\subsection{Setting}\label{context}

Suppose we measure an indicator of a policy $A_{it}$, a possibly affected outcome $Y_{it}$, and pre-treatment covariates $\bX_{it}$---which may be time-invariant or time-varying---for $i = 1, ..., n$ units (often the units are geographic entities, like states, counties, etc.) and $t = 1, ..., p$ timepoints. We write $\bX_i$ when covariates are time-invariant or when the period is clear from context; the distinction matters for Path~3 (Section~\ref{sec:covariate-transport}), where a shifting covariate distribution is the engine of extrapolation. For ease of exposition, we take $A_{it}$ to be a policy, but all that follows applies equally to other types of interventions. Define the counterfactual value that $Y_{it}$ would take if the policy were set, possibly contrary to fact, to $A_{it} = a$ as $Y_{it}(a)$. We assume no interference: each unit's potential outcomes depend only on that unit's treatment, so $Y_{it}(a)$ is well-defined. We further assume throughout that the covariates $\bX_{it}$ used for adjustment are \emph{unaffected by the policy}---pre-treatment or otherwise externally determined, so that $\bX_{it}$ does not lie on a causal path from treatment to outcome. With time-invariant baseline covariates this holds automatically; it is the standard condition underlying covariate adjustment in any first-stage design and is invoked again by Path~3 (Section~\ref{sec:covariate-transport}), which integrates over a \emph{future} covariate distribution. When covariates are time-varying \emph{and} affected by treatment, valid adjustment requires g-methods \citep{robinsNewApproachCausal1986}; we assume exogeneity and leave that case to future work. Let $\cO = \{\bX_i, \bY_{i}(0), \bY_{i}(1), \bA_{i}\}_{i = 1, ..., n} \sim \P$ where $\bY_i(a) = \{\bY_{it}(a)\}_{t = 1, ..., p}$ and $\bA_i = (A_{it})_{t = 1,.., p}$. We extend the setting to future time periods when defining the future effect family below. The distribution of data at time $t$ is $\P_t$. For integer $k \geq 1$, we write $\P_{p+k}$ for the distribution at the future period $p+k$; nothing in the observed data identifies $\P_{p+k}$ without further assumption, which is the identification problem this section states and the remainder of the paper solves three different ways.
...
Assumption~\ref{absorbing-adoption} states that adoption is permanent over the window under study: a policy in force at $t$ remains in force through $p+1$, so $G_i = \min\{t : A_{it}=1\}$ records the single date at which unit $i$ transitions, and $A_{it} = \mathbf{1}\{G_i \leq t\}$. What it rules out is repeal and reinstatement within $\{1,\ldots,p+1\}$; it says nothing about which units adopt or when. Assumption~\ref{absorbing-adoption} is maintained throughout and is the only assumption behind Lemmas~\ref{lem:persistence} and~\ref{lem:aggregation} below (Section~\ref{sec:effect-level-invariance}).

\paragraph{The identification argument has two steps, and only the second is new.} The remainder of this paper assumes that the researcher has secured identification of the ATT $\theta$ and, where relevant, the group-time ATTs $\theta_{gt}$, under some design. For example, under parallel trends in difference-in-differences, $\theta_{gt}$ is identified from the observed data \citep{callawayDifferenceindifferencesMultipleTime2021,sunEstimatingDynamicTreatment2021}; under unconfoundedness or other designs, different identification formulas apply. We do not restrict the first-stage identification strategy: it may be difference-in-differences, synthetic control, or another approach. The contribution of this paper is the second step: showing when and how these backward-looking estimands can be used to identify the policy-relevant future effects defined next, given time homogeneity assumptions (Section~\ref{sec:effect-level-invariance}), a parametric model for dynamics (Section~\ref{sec:parametric-time-path}), or structural covariate-driven effects (Section~\ref{sec:covariate-transport}). The full identification argument thus has two steps: (a) under the researcher's design, $\theta$ or $\theta_{gt}$ are identified from observables; (b) under the extrapolation assumptions stated in Sections~\ref{sec:effect-level-invariance}--\ref{sec:covariate-transport}, the future effect is identified from $\theta$ or $\theta_{gt}$.
...
\subsection{The future effect family}\label{sec:effect-family}
...
These three estimands---FATT, FATU, and FATE---are the policy-relevant family we carry through the paper. They map directly to the three decisions a policymaker faces: whether to \emph{repeal} a policy currently in place (FATT), whether to \emph{adopt} one not yet in place (FATU), and the effect of \emph{universal} adoption (FATE). They also share identification machinery: each is the future conditional effect $\tau(x)$ averaged over a different target covariate distribution (treated units, untreated units, or the whole population), a structure we exploit directly in Path~3 (Section~\ref{sec:covariate-transport}) and which the FATE/FATU recover from the FATT via the relationships above. We develop identification primarily for the FATT and indicate throughout how the FATU and FATE follow.

Unlike the ATT and sub-ATTs above, none of the counterfactuals are observed. In fact, unlike for any backward-looking estimand, the future period's distribution $\P_{p+k}$ is not merely unobserved but wholly unknown: no amount of data on $\{1,\ldots,p\}$ pins it down without an added invariance assumption connecting it to the observed window. We therefore require additional machinery to connect the observed data to the policy-relevant distribution $\P_{p+k}$. Sections~\ref{sec:effect-level-invariance}--\ref{sec:covariate-transport} supply three such assumptions, each buying that missing distribution from historical data in a different way: taking time heterogeneity \textit{less} seriously, by making time homogeneity assumptions (Section~\ref{sec:effect-level-invariance}); or taking it \textit{more} seriously, either by formally modeling how policy effects change over time (Section~\ref{sec:parametric-time-path}) or by modeling structural, covariate-conditional heterogeneity and transporting it to the future covariate distribution (Section~\ref{sec:covariate-transport}). \textbf{In the remainder of the paper we take $k=1$ (one period ahead);} extension to $k > 1$ is analogous and noted where it differs.

\section{Effect-Level Invariance: Weakening and Strengthening Dynamics}\label{sec:effect-level-invariance}

This section develops the first of the three routes: holding the \emph{level} of the group-time treatment effects invariant across the transition from the observed window to $p+1$. We develop it as a spectrum rather than a single assumption. Section~\ref{sec:path1} states the identifying assumption at its natural strength -- constant across both cohort and time -- establishes the identification result, and then weakens the assumption to allow cohort-specific effect levels, degrading the exact answer to a share-weighted convex combination. Section~\ref{sec:strengthening} runs the same kind of assumption the other way: stated once, generically, over an arbitrary conditioning event, strengthening it collapses the \emph{entire} future effect family -- treated, untreated, population, similar-units, even unit-level -- onto the single backward-looking ATT. The two directions belong in one section because the strengthening result is the limiting case of the weakening spectrum, not an independent idea.
...
\subsection{Weakening: from strict to within-group time homogeneity}\label{sec:path1}
...
\paragraph{Weakening across cohorts.} Assumption~\ref{strict-time-homogeneity} restricts treatment effects to be constant along two margins at once -- within a cohort over time, and across cohorts. A great deal of recent literature in causal inference for panel data has explicitly rejected constancy across cohorts, positing that treatment effects may vary by when the treatment was first implemented \citep{sunEstimatingDynamicTreatment2021,brownDynamicTreatmentEffect2025}; proposals for group-time effects $\theta_{gt}$ capture this by encoding adoption timing in $G_i = g$. We weaken Assumption~\ref{strict-time-homogeneity} along exactly this margin, requiring homogeneity only within cohorts and leaving effect magnitudes free to differ across them:
...
We take $G_i$ to be a function of $A_i$ (the time of first adoption), so that $\omega_g$ is the distribution of cohorts among units treated by period $p$. Corollary~\ref{cor:path1-within} states that when within-cohort ATTs are not dynamic, a convex combination of cohort-specific ATTs extrapolates the FATT to $p+1$, with the cohort shares fixed at the last observed period. This is precisely the "weakening" half of this section's spectrum: dropping cross-cohort constancy costs the exact equality $\theta_{p+1}=\theta$ and replaces it with a share-weighted average, at no cost to identification as long as each cohort-specific $\theta_{g\cdot}$ is itself identified.

\subsection{Strengthening: collapsing the future family onto the backward ATT}\label{sec:strengthening}

The other direction runs the homogeneity assumption the opposite way. Rather than relaxing cross-cohort constancy, suppose we further restrict between-unit heterogeneity beyond what Assumption~\ref{strict-time-homogeneity} already imposes; the entire future effect family then collapses onto the single backward-looking ATT, and it is straightforward to see why: if effects have no between-state heterogeneity at $p+1$, treated, untreated, and population future effects are all values of the same constant. Consider assuming that there is no between-state heterogeneity in treatment effects at time $p+1$:
...
\]
\end{assumption}
Under Assumptions \ref{general-strict-time-homogeneity} and \ref{general-p-to-p-plus-one-heterogeneity}, taking $\cC_t = \{A_{it} = 0\}$ yields equivalence between the backward-looking ATU and the FATU; taking $\cC_t = \emptyset$ yields equivalence between the backward-looking ATE and the FATE; and taking $\cC_t = \{\rho(\bX_i, \bX_j) \leq \eta\}$ yields equivalence between the backward-looking ATS and the FATS (Assumption \ref{general-p-to-p-plus-one-heterogeneity} not required for the FATS). This is the sense in which strengthening the homogeneity assumption of Section~\ref{sec:path1} -- rather than weakening it, as that section's within-group corollary does -- is what applied practice is implicitly asserting whenever it reads a historical ATT, ATU, or ATE as today's corresponding future effect.

\section{A Parametric Time Path for the Group-Time Effects}\label{sec:parametric-time-path}
...
\section{Covariate Transport: A Lucas-Critique Route}\label{sec:covariate-transport}
...
The contribution of this subsection is to derive the influence function (or, for Path~3, the score) for the policy-relevant FATT $\theta_{p+1}$ from an asymptotically linear first-stage estimator of the relevant object, so that valid standard errors propagate automatically wherever propagation is possible. For Paths~1 and~2 that object is the group-time ATTs $\theta_{gt}$, and the FATT is a smooth finite-dimensional function of them, so an influence function propagates by linearity or the chain rule. Path~3 is structurally different, and we say so rather than obscuring it: its first-stage object is the conditional effect function $\tau(\cdot)$, which is generally not root-$n$ estimable, so no influence function is propagated for it. In its place we develop a Neyman-orthogonal doubly-robust transport score for the scalar target $\theta_{p+1}$ directly, and recover root-$n$ inference for that scalar through orthogonality with cross-fitted nuisances -- the same destination as Paths~1 and~2, reached by a different mechanism. We present Paths~1 and~2 first, then Path~3.

\paragraph{Path 1 (time homogeneity).} Under within-group time homogeneity, $\theta_{p+1} = \sum_g \omega_g \theta_{g\cdot}$ with $\omega_g = \P(G_i = g | A_{ip} = 1)$. Take as given the EIF $\phi_{\theta_{g\cdot}}$ for each $\theta_{g\cdot}$ (e.g., averaged over $t$ in the observed window, or as provided by the first-stage). How that within-cohort average should be formed is not innocuous when a cohort contributes several group-time cells whose influence functions are correlated---as they are whenever the first stage draws on overlapping comparison units, for instance a not-yet-treated control group \citep{callawayDifferenceindifferencesMultipleTime2021}. Section~\ref{sec:md-aggregation} treats that aggregation step explicitly; the display below holds for whatever asymptotically linear $\widehat{\theta}_{g\cdot}$ results. By linearity (see Appendix \ref{app:eif1}), the EIF for $\psi_1 = \theta_{p+1}$ is
\begin{align*}
...
\paragraph{Path 3 (covariate transport): a Neyman-orthogonal score, not a propagated EIF.} Path~3 supports two distinct inferential regimes. They correspond to different information sets and therefore have different efficient influence functions.

\emph{Regime (i): an internally defined target.}
...
\emph{Regime (iii): a known or fixed density ratio.}
...
\subsection{Aggregating correlated cells}\label{sec:md-aggregation}

Paths~1 and~2 both reduce the FATT to a linear functional of a lower-dimensional parameter that the observed group-time cells overidentify, and both displays above take the aggregation across cells as given. This subsection makes it explicit, because the cells' influence functions are in general correlated and the correlation is not a nuisance to be ignored: cells sharing comparison units---most visibly when the first stage uses a not-yet-treated control group \citep{callawayDifferenceindifferencesMultipleTime2021}---have genuinely dependent scores, and an aggregation that assumes independence is neither efficient nor correctly sized.

\paragraph{Stacked scores.} Let $\cC := \{(g,t) : g \in \cG,\ g \leq t \leq p\}$ index the $J := |\cC|$ observed cells, and collect the first-stage influence functions of Assumption~\ref{RC2} into the random vector
\[
\bphi_i := (\phi_{gt,i})_{(g,t)\in\cC} \in \R^{J},
\qquad
\bSigma := \Var(\bphi_i) = \E[\bphi_i\bphi_i^{\top}].
\]
Assumption~\ref{RC2} is a marginal statement, cell by cell, but no separate joint condition is needed: every $\phi_{gt,i}$ is a square-integrable function of the same unit $i$'s observed data, and the units are i.i.d., so for any fixed $\ba \in \R^{J}$ the scalar $\ba^{\top}\bphi_i$ is a mean-zero i.i.d.\ average and $\ba^{\top}\{\sqrt n(\bthetahat - \btheta)\} \leadsto N(0, \ba^{\top}\bSigma\ba)$. Cram\'er--Wold then delivers joint asymptotic normality of the stacked vector $\sqrt n(\bthetahat-\btheta) \leadsto N(\vzero,\bSigma)$ with no assumption beyond Assumption~\ref{RC2}. What $\bSigma$ is \emph{not} is diagonal, and that is the content of this subsection.

\paragraph{The restriction and the target.} Suppose the identifying assumption of the chosen path restricts the cells to a linear model in a $d$-dimensional parameter,
\[
\theta_{gt} = \bx_{gt}^{\top}\bgamma
\quad \text{for all } (g,t)\in\cC,
\qquad\text{equivalently}\qquad
\btheta = X\bgamma,
\]
with $X$ the $J \times d$ matrix of rows $\bx_{gt}^{\top}$ of Corollary~\ref{cor:linear-rank}, and let
\[
\bx_{p+1} := \sum_{g\in\cG}\omega_g\,\bx_{g,p+1},
\qquad\text{so that}\qquad
\theta_{p+1} = \bx_{p+1}^{\top}\bgamma .
\]
Two cases matter here. Path~1 is $\bx_{gt} = \be_g$, the indicator of cohort $g$, with $d = q := |\cG|$, $\bgamma = (\theta_{g\cdot})_{g\in\cG}$, and $\bx_{p+1} = \bomega$; the restriction $\btheta = X\bgamma$ is then exactly Assumption~\ref{within-group-strict-time-homogeneity} on the observed window. Path~2 with $f$ linear in $\bgamma$ is $\bx_{gt}$ as in Corollary~\ref{cor:linear-rank} and $\bx_{p+1} = \sum_g \omega_g \bx_{g,p+1}$, which covers the event-time and spline specifications used in Section~\ref{sec:model-selection} and in the simulations. In neither case does the efficient combination factor into a within-cohort step followed by an across-cohort step, unless $\bSigma$ happens to be block diagonal across cohorts.

\begin{proposition}[Efficient aggregation of correlated cells]\label{prop:md-aggregation}
Let Assumptions~\ref{RC1}--\ref{RC3}, \ref{RC6} and \ref{RC11} hold, let $\btheta = X\bgamma$ with $X$ of full column rank $d$ (Assumption~\ref{RC5}(ii) and Corollary~\ref{cor:linear-rank}), and set
\[
H := X^{\top}\bSigma^{-1}X,
\qquad
\bgammahat := H_n^{-1}X^{\top}\bSigmahat^{-1}\bthetahat,
\qquad
H_n := X^{\top}\bSigmahat^{-1}X .
\]
Then the following hold.
\begin{enumerate}
\item[(i)] \emph{Asymptotic linearity.} $\bgammahat$ is asymptotically linear with influence function $H^{-1}X^{\top}\bSigma^{-1}\bphi_i$, and for known $\bomega$ the estimator $\widehat{\theta}_{p+1} := \bx_{p+1}^{\top}\bgammahat = \blambda^{*\top}\bthetahat$ with
\[
\blambda^{*} := \bSigma^{-1}X H^{-1}\bx_{p+1} \in \R^{J}
\]
is asymptotically linear with influence function
\[
\phi_{p+1,i} \;=\; \blambda^{*\top}\bphi_i \;=\; \bx_{p+1}^{\top}H^{-1}X^{\top}\bSigma^{-1}\bphi_i,
\qquad
\Var\!\left(\sqrt n\,\widehat{\theta}_{p+1}\right) \to \bx_{p+1}^{\top}H^{-1}\bx_{p+1}.
\]
If $\bomega$ is estimated, the influence function acquires the additional term $\sum_{g\in\cG} \bx_{g,p+1}^{\top}\bgamma\,\phi_{\omega_g,i}$, exactly as in the Path~1 and Path~2 displays of Section~\ref{sec:eif-derivation}.
\item[(ii)] \emph{Unbiasedness of the combination.} $\blambda^{*\top}X = \bx_{p+1}^{\top}$, so $\blambda^{*\top}\btheta = \theta_{p+1}$ for every $\bgamma$ satisfying the restriction.
\item[(iii)] \emph{Optimality within the class.} Among all fixed $\blambda \in \R^{J}$ satisfying the unbiasedness constraint $\blambda^{\top}X = \bx_{p+1}^{\top}$, the asymptotic variance $\blambda^{\top}\bSigma\blambda$ is uniquely minimized at $\blambda = \blambda^{*}$, with minimum $\bx_{p+1}^{\top}H^{-1}\bx_{p+1}$.
\item[(iv)] \emph{Independent cells.} If $\bSigma = \mathrm{diag}(\sigma^2_{gt})$ and the restriction is within-cohort constancy, $\blambda^{*}$ reduces to inverse-variance weighting: within each cohort $g$, cell $(g,t)$ receives weight proportional to $\sigma_{gt}^{-2}$, and the cohort averages are combined with weights $\omega_g$.
\end{enumerate}
\end{proposition}

\begin{proof}
See Appendix~\ref{app:prop-md}.
\end{proof}

\begin{remark}[What is optimized, and what is not]\label{rem:md-efficiency}
Proposition~\ref{prop:md-aggregation}(iii) is an optimality statement inside a stated class, and the class matters. The comparison set is the fixed linear combinations of the \emph{supplied} cell influence functions that are unbiased for $\theta_{p+1}$ under the restriction; within that set $\blambda^{*}$ attains $\bx_{p+1}^{\top}(X^{\top}\bSigma^{-1}X)^{-1}\bx_{p+1}$, which is the efficient minimum-distance---equivalently, optimal-GMM---variance for the linear moment system $\E[\bthetahat - X\bgamma] = \vzero$. It is \emph{not} a claim that $\widehat{\theta}_{p+1}$ attains the semiparametric efficiency bound for the FATT. Attaining that bound would require three further things that Proposition~\ref{prop:md-aggregation} neither assumes nor delivers: that the efficient influence function for $\theta_{p+1}$ in the restricted model lie in the linear span of the $J$ supplied cell influence functions; that the cell set $\cC$ and the first-stage design be themselves an efficient use of the data; and that each supplied $\phi_{gt}$ be efficient for its own $\theta_{gt}$. Each can fail independently of the others, and the first fails generically whenever the restricted model's tangent space is not spanned by the chosen cells. The honest reading of the proposition is therefore conditional: given the cells the researcher has and the first-stage scores that came with them, this is the best fixed reweighting of those scores, and it dominates the diagonal weighting of part~(iv) exactly to the extent that $\bSigma$ has nonzero off-diagonal entries. The parallel hedge in Appendix~\ref{app:eif1} is the same statement made cell by cell.
\end{remark}

\begin{remark}[Estimated cohort shares break the optimality, not the validity]\label{rem:md-omega}
Part~(iii) is stated for known $\bomega$ for a reason. When $\bomega$ is estimated, the influence function of $\widehat{\theta}_{p+1}$ is $\blambda^{\top}\bphi_i + r_i$ with $r_i := \sum_g \bx_{g,p+1}^{\top}\bgamma\,\phi_{\omega_g,i}$, and minimizing $\Var(\blambda^{\top}\bphi_i + r_i)$ subject to $\blambda^{\top}X = \bx_{p+1}^{\top}$ is not the problem solved by $\blambda^{*}$ unless $\bsigma := \Cov(\bphi_i, r_i) = \vzero$. The constrained minimizer is $\blambda^{\dagger} = \blambda^{*} - \bSigma^{-1}(I_J - X H^{-1}X^{\top}\bSigma^{-1})\bsigma$, which differs from $\blambda^{*}$ by a correction in the $\bSigma^{-1}$-orthogonal complement of the column space of $X$. Using $\blambda^{*}$ when $\bsigma \neq \vzero$ therefore costs efficiency but not validity: part~(i) still gives the correct influence function and hence correctly sized inference, because unbiasedness of the combination, part~(ii), does not involve $\bsigma$. We use $\blambda^{*}$ throughout and do not claim optimality in this case.
\end{remark}

\begin{proposition}[Asymptotic distribution of $\widehat{\theta}_{p+1}$]\label{prop:asymp}
Under the identifying assumptions of the applicable path and Assumptions~\ref{RC1}--\ref{RC11} relevant to that path, suppose the stated influence functions have finite, positive variance.
...
\section{Selecting a Temporal Model by Extrapolation, Not Fit}\label{sec:model-selection}
...
\end{figure}
When the slope in event time is non-zero, the two curves in Figure~\ref{fig:backward-vs-fatt} separate; interpreting the backward-looking ATT as the policy-relevant effect is then misleading. This motivates the need for time-homogeneity assumptions (Path~1), explicit temporal extrapolation (Path~2), or covariate-driven effects (Path~3); Appendix~\ref{app:additional-sims} reports Path~1's behavior in isolation (Path~2's correctly-specified-versus-misspecified comparison follows directly below, since it bears on this section's own claims). Here we go directly to a head-to-head comparison and to the validity of the propagated inference.
...
Misspecification is the sharper test of Path~2's own numbered claim -- that a wrong functional form does not merely inflate variance but converges to the wrong number (Section~\ref{sec:parametric-time-path}) -- so we check it directly rather than deferring it to supplementary material. Path~2 fits a parametric model $f(g,t;\widehat{\bgamma})$ to $\widehat{\theta}_{gt}$ and extrapolates to $p+1$.
\input{sim_tables/section3}
Correct specification (linear DGP, linear fit) yields low bias and nominal coverage. Misspecification (quadratic DGP, linear fit) introduces bias and can distort coverage; fitting the correct quadratic model restores performance. This illustrates that Path~2 trades robustness for point identification and that the choice of $f$ matters in practice -- precisely the motivation for the cross-validation criterion of Section~\ref{sec:model-selection}.
...
\input{section7.7_cv_simulation}

\input{section9_application}

\section{Discussion}\label{sec:discussion}

We have argued that policy analysis with panel data faces a tension: methods that allow dynamic, group-time effects address important identification concerns but produce backward-looking estimands that do not directly answer whether to maintain or adopt a policy now or next period. We resolve this by defining future causal effects (FATT, FATU, FATE, FATS) and showing that three apparently unrelated identification strategies are in fact one framework applied three times: each extrapolates a different choice of what is held invariant across the transition from the observed window to $p+1$, plugged into the same forward map from the observed group-time effects to the future estimand. \textbf{Path~1 (time homogeneity):} holding the effect level itself invariant, the FATT is identified by the backward-looking ATT or by a convex combination of group-specific ATTs; the researcher can use standard group-time or ATT estimators and aggregate. \textbf{Path~2 (temporal extrapolation):} holding the shape of the temporal path invariant, the FATT is identified by fitting a parametric model $f(g,t;\bgamma)$ to the identified group-time ATTs and extrapolating to $p+1$. \textbf{Path~3 (covariate integration):} holding the covariate-conditional effect invariant instead of anything about time, the FATT is identified by estimating the conditional effect $\tau(x)$ and integrating over the future covariate distribution. Inference follows the same division: for Paths~1 and~2 the influence function of the future effect is propagated from the first-stage estimator by linearity or the chain rule, with no route-specific asymptotic theory re-derived; Path~3 is the one honestly-flagged exception, because the conditional effect function it relies on is generally not root-$n$ estimable, so we develop a Neyman-orthogonal doubly-robust transport score for the scalar target instead of propagating an influence function for the underlying function. The choice between paths is substantive, not mechanical: Path~1 is appropriate when temporal stability is plausible; Path~2 when the researcher is willing to model temporal dynamics and accept sensitivity to the choice of $f$; Path~3 when covariate-driven heterogeneity is well-documented and the researcher believes $\tau(x)$ is regime-invariant.

The application in Section~\ref{sec:application} puts this structural picture against a genuine out-of-sample test, and the result reconciles with it rather than contradicting it. All three paths underpredicted the realized effect of stand-your-ground laws in the validation window, none achieved nominal coverage, and the covariate route (Path~3) performed \emph{worse} than the simplest homogeneity route (Path~1) -- evidence that the baseline covariates available carried little explanatory power for the effect heterogeneity actually present. This is not a failure of the framework's own logic: each path correctly extrapolates under its stated invariance assumption, and the application shows what happens when none of those assumptions holds well enough in a specific setting. The lesson is about what the framework promises. It makes the extrapolation assumption explicit and, on the observed window, partially checkable (Remark~\ref{rem:testable} and its Path-3 analog); it does not make extrapolation itself safe. When the training-period structure does not persist -- as it evidently did not here -- no path is rescued by additional methodological sophistication, and the honest response is to report the failure rather than search for the path that happens to look best in-sample.

Several limitations are stated here as the paper's own boundaries, not as reviewer-anticipated hedges. The paper proposes no new first-stage identification strategy: the backward-looking ATT, the group-time ATTs, or the conditional effect function are taken as identified under whatever design the researcher brings (parallel trends, unconfoundedness, synthetic control), and only the extrapolation step -- connecting those identified objects to a future period -- is new (Section~\ref{context}). Identification of the FATT in all paths relies on the distribution $\P_{p+1}$ (and, for Paths~2 and~3, on the extrapolation or conditional model holding at $p+1$), which is not directly observable; we have focused on a single future period ($k=1$), and extension to multiple horizons $k>1$ is conceptually straightforward but adds assumptions on how far the model can be projected that we do not discharge here. Throughout, covariates are assumed unaffected by the policy (Section~\ref{context}), a condition Path~3's transport target makes bind hardest (Section~\ref{sec:covariate-transport}); time-varying, policy-affected covariates would require g-methods, which we leave to future work. We point-identify the FATT rather than bound it: partial identification and sensitivity analysis under violations of the invariance assumptions are named as the alternative we are not taking, not one we have ruled out as inferior. Inference uses the propagated efficient influence function or transport score, which is valid under correct specification of the first-stage and, for Paths~2 and~3, of the model ($f$ or $m$); misspecification can bias the extrapolated estimate and distort inference. Finally, the CV procedure of Section~\ref{sec:model-selection} introduces model-selection randomness that our influence functions condition away rather than account for -- a genuine open problem, not an oversight, since post-selection inference here would need to average over the same held-out-period splits the procedure already uses.
...
\right]}{n}.
\]
Inference in that regime must account for both terms; it is not covered by the internally targeted or known/fixed-density-ratio results presented in Section~\ref{sec:covariate-transport}.
...
\input{sim_tables/section8_3_combined}
...
\input{section7.7b_cv_robustness}
...
\begin{assumption}[Nonsingular and estimable score covariance]\label{RC11}
For the aggregation of Section~\ref{sec:md-aggregation}, with $\bphi_i := (\phi_{gt,i})_{(g,t)\in\cC}$ and $\bSigma := \Var(\bphi_i)$ over the finite, fixed cell set $\cC$:
\begin{enumerate}
\item[(i)] $\bSigma \succ 0$, that is, $\Var(\ba^{\top}\bphi_i) > 0$ for every $\ba \neq \vzero$; equivalently, no nontrivial linear combination of the cell influence functions is degenerate.
\item[(ii)] There is an estimator with $\bSigmahat \to_{\P} \bSigma$, for instance the sample covariance $n^{-1}\sum_i \widehat{\bphi}_i\widehat{\bphi}_i^{\top}$ of the estimated scores aligned on a common unit index.
\end{enumerate}
Clause~(i) strengthens Assumption~\ref{RC2}, which bounds each $\E[\phi_{gt,i}^2]$ marginally but permits exact linear dependence across cells; it is what makes $\bSigma^{-1}$ and $X^{\top}\bSigma^{-1}X$ well defined. No separate joint-asymptotic-linearity condition is imposed: because every $\phi_{gt,i}$ is a function of the same unit $i$'s data and the units are i.i.d., joint asymptotic normality of the stacked vector follows from Assumption~\ref{RC2} by the Cram\'er--Wold theorem. The full-column-rank requirement on $X$ is not new either: Assumption~\ref{RC5}(ii) is stated for an arbitrary positive weight matrix $\Lambda$, and $\Lambda = \bSigma^{-1}$ is one, so under clause~(i) the nonsingularity of $H = X^{\top}\bSigma^{-1}X$ is exactly Assumption~\ref{RC5}(ii) (Corollary~\ref{cor:linear-rank} for linear $f$).
\end{assumption}

\begin{remark}
Assumptions~\ref{RC1}--\ref{RC3} and \ref{RC6} govern the asymptotically linear first stages used by Paths~1 and~2. Assumptions~\ref{RC4}--\ref{RC5} are specific to Path~2. Assumptions~\ref{RC8}--\ref{RC10} distinguish the two Path~3 information regimes: internally observed target membership and a known or conditionally fixed density ratio. Assumption~\ref{RC7} records the $o_{\P}(n^{-1/2})$ product-rate requirement used with cross-fitting, following \citet{chernozhukovDoubleDebiasedMachine2018}. Assumption~\ref{RC11} governs Section~\ref{sec:md-aggregation}'s efficient aggregation across correlated cells for Paths~1 and~2.
\end{remark}
...
\textbf{(4)} \textit{Validity and efficiency:} $\widehat{\theta}_{p+1}$ is asymptotically linear with the stated influence function, so the propagated variance is valid for any asymptotically linear first-stage estimators of $\theta_{g\cdot}$. It attains the semiparametric efficiency bound for $\theta_{p+1}$ only under the further conditions catalogued in Remark~\ref{rem:md-efficiency}; efficiency of the supplied $\phi_{\theta_{g\cdot}}$ and of the weights $\widehat{\omega}_g$ is necessary for that but not sufficient, since it says nothing about whether the cell set spans the restricted model's tangent space or whether the within-cohort aggregation over $t$ is itself efficient---the question Proposition~\ref{prop:md-aggregation} answers.
...
\subsection{Proof of the Efficient-Aggregation Proposition}\label{app:prop-md}

We prove Proposition~\ref{prop:md-aggregation}.

\begin{proof}
\textit{Roadmap:} the estimator is an exactly-identified linear map of $\bthetahat$, so asymptotic linearity is algebra plus consistency of $\bSigmahat$; optimality is a one-line completion of the square in the $\bSigma$ inner product, which simultaneously delivers the unbiasedness identity and the variance identity.

\textbf{Step 1 (joint expansion).} By Assumption~\ref{RC2}, $\sqrt n(\bthetahat - \btheta) = n^{-1/2}\sum_i \bphi_i + o_{\P}(1)$ coordinatewise, hence in $\R^{J}$ since $J$ is finite and fixed. Each $\bphi_i$ is a function of unit $i$'s data alone and the units are i.i.d.\ with $\E[\bphi_i]=\vzero$ and, by Assumption~\ref{RC6}, finite second moments; so for any fixed $\ba$, the classical CLT gives $\ba^{\top}n^{-1/2}\sum_i\bphi_i \leadsto N(0,\ba^{\top}\bSigma\ba)$, and Cram\'er--Wold gives $\sqrt n(\bthetahat - \btheta) \leadsto N(\vzero,\bSigma)$. In particular $\bthetahat - X\bgamma = O_{\P}(n^{-1/2})$ under the restriction.

\textbf{Step 2 (asymptotic linearity of $\bgammahat$).} Since $X$ has full column rank $d$ and $\bSigmahat \succ 0$ with probability approaching one by Assumption~\ref{RC11}, $H_n$ is invertible on that event and, using $X^{\top}\bSigmahat^{-1}X H_n^{-1} = I_d$,
\[
\bgammahat - \bgamma
= H_n^{-1}X^{\top}\bSigmahat^{-1}\bthetahat - \bgamma
= H_n^{-1}X^{\top}\bSigmahat^{-1}\left(\bthetahat - X\bgamma\right),
\]
an exact identity requiring no expansion. By Assumption~\ref{RC11}(ii), $H_n^{-1}X^{\top}\bSigmahat^{-1} = H^{-1}X^{\top}\bSigma^{-1} + o_{\P}(1)$, and this multiplies an $O_{\P}(n^{-1/2})$ vector by Step~1, so the substitution costs $o_{\P}(n^{-1/2})$:
\[
\sqrt n(\bgammahat - \bgamma) = n^{-1/2}\sum_i H^{-1}X^{\top}\bSigma^{-1}\bphi_i + o_{\P}(1).
\]
Estimating the weight matrix therefore does not affect the first-order limit, the standard two-step minimum-distance conclusion. Part~(i)'s display for $\widehat{\theta}_{p+1}$ follows by the fixed linear map $\bx_{p+1}^{\top}$ when $\bomega$ is known, since then $\bx_{p+1}^{\top}H^{-1}X^{\top}\bSigma^{-1} = \blambda^{*\top}$ by the definition of $\blambda^{*}$. When $\bomega$ is estimated, apply the delta method to $(\bgamma,\bomega)\mapsto \sum_g \omega_g\bx_{g,p+1}^{\top}\bgamma$ and use Assumption~\ref{RC3}; the $\partial/\partial\omega_g$ terms are the stated $\sum_g \bx_{g,p+1}^{\top}\bgamma\,\phi_{\omega_g,i}$.

\textbf{Step 3 (part (ii), unbiasedness).} Directly,
\[
\blambda^{*\top}X = \bx_{p+1}^{\top}H^{-1}X^{\top}\bSigma^{-1}X = \bx_{p+1}^{\top}H^{-1}H = \bx_{p+1}^{\top},
\]
so $\blambda^{*\top}\btheta = \bx_{p+1}^{\top}\bgamma = \theta_{p+1}$ whenever $\btheta = X\bgamma$.

\textbf{Step 4 (part (iii), optimality and the variance identity).} Let $\blambda$ satisfy $\blambda^{\top}X = \bx_{p+1}^{\top}$ and write $\bdelta := \blambda - \blambda^{*}$, so that $X^{\top}\bdelta = \vzero$ by Step~3. Since $\bSigma\blambda^{*} = X H^{-1}\bx_{p+1}$, the cross term vanishes:
\[
\bdelta^{\top}\bSigma\blambda^{*} = \bdelta^{\top}X H^{-1}\bx_{p+1} = \vzero^{\top}H^{-1}\bx_{p+1} = 0 .
\]
Hence
\[
\blambda^{\top}\bSigma\blambda
= \blambda^{*\top}\bSigma\blambda^{*} + \bdelta^{\top}\bSigma\bdelta,
\qquad
\blambda^{*\top}\bSigma\blambda^{*}
= \bx_{p+1}^{\top}H^{-1}X^{\top}\bSigma^{-1}\bSigma\bSigma^{-1}X H^{-1}\bx_{p+1}
= \bx_{p+1}^{\top}H^{-1}\bx_{p+1},
\]
the second display being the variance identity asserted in part~(i). Because $\bSigma \succ 0$ by Assumption~\ref{RC11}(i), $\bdelta^{\top}\bSigma\bdelta \geq 0$ with equality if and only if $\bdelta = \vzero$, so the minimum is attained uniquely at $\blambda^{*}$. This is the efficient minimum-distance, equivalently optimal-GMM, weighting for the moment system $\E[\bthetahat - X\bgamma] = \vzero$: the optimal weight matrix is the inverse of the moment covariance.

\textbf{Step 5 (part (iv), the diagonal case).} Suppose $\bSigma = \mathrm{diag}(\sigma^2_{gt})$ and $\bx_{gt} = \be_g$, so $X$ is the matrix of cohort indicators and $d = q$. Then $X^{\top}\bSigma^{-1}X = \mathrm{diag}\big(\sum_{t : (g,t)\in\cC}\sigma_{gt}^{-2}\big)_{g\in\cG}$ is diagonal, so with $\bx_{p+1} = \bomega$,
\[
\lambda^{*}_{gt}
= \omega_g\,\frac{\sigma_{gt}^{-2}}{\sum_{s : (g,s)\in\cC}\sigma_{gs}^{-2}} .
\]
Within each cohort the weights are proportional to $\sigma_{gt}^{-2}$ and sum to one, and the cohort aggregates carry weight $\omega_g$: inverse-variance weighting within cohorts followed by $\bomega$-weighting across them. The two-step form is a consequence of $X^{\top}\bSigma^{-1}X$ being diagonal, which fails in general when $\bSigma$ has nonzero entries linking cells of different cohorts.
\end{proof}

\paragraph{Post-proof audit.}
Assumption~\ref{RC11}(i) is used twice and both uses are essential: to define $\bSigma^{-1}$ and $H$, and to get the strict inequality that makes the minimizer in Step~4 unique. Assumption~\ref{RC11}(ii) is used only in Step~2, and only to replace $\bSigmahat$ by $\bSigma$ inside a product with an $O_{\P}(n^{-1/2})$ term; no rate on $\bSigmahat$ beyond consistency is required, and no Donsker condition enters, because the cell set is finite and fixed and $X$ is nonrandom. The full-column-rank requirement on $X$ is Assumption~\ref{RC5}(ii) with $\Lambda = \bSigma^{-1}$, not a new condition. The identity in Step~2 is exact rather than a first-order expansion, so no second-order remainder is being suppressed. Two limits of the result should be read off explicitly. First, the optimality in Step~4 is over fixed $\blambda$ within the constraint set $\{\blambda : \blambda^{\top}X = \bx_{p+1}^{\top}\}$ only; it says nothing about estimators outside the span of the supplied cell scores, and is therefore not a semiparametric efficiency claim for $\theta_{p+1}$ (Remark~\ref{rem:md-efficiency}). Second, Step~4 assumes $\bomega$ known; when $\bomega$ is estimated with influence function correlated with $\bphi_i$, part~(i) still holds but part~(iii) does not, and the constrained minimizer is the $\blambda^{\dagger}$ of Remark~\ref{rem:md-omega}. Nonlinearity of $f$ in $\bgamma$ is outside the scope of the proposition: the restriction is assumed linear, so the construction applies to Path~1 always and to Path~2 only when $f$ is linear in $\bgamma$, for which Corollary~\ref{cor:linear-rank} supplies the rank condition.
...
o_{\P}(1),
\]
with $\phi_{\mathrm{int}}$ as displayed in Section~\ref{sec:eif-derivation}; for the FATT, $T=A$, $\rho=\pi$, $q=e$ gives \eqref{eq:eif-path3}, and the two remainder terms above coincide ($\delta_q=\delta_e$), so only $\|\widehat\mu_0-\mu_0\|_2\|\widehat e-e\|_2=o_{\P}(n^{-1/2})$ is needed in that case.
