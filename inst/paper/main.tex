...
In many applications covariates are time-invariant, so the target is simply the empirical distribution of $\bX_i$ among treated units in the sample and we may take $\bX_p$ as the target. When covariates are time-varying, $\bX$ may be forecasted to $p+1$ (population growth, economic indicators); sometimes $\bX_{p+1}$ is directly observed. The boundedness of $w$ is the overlap condition for transport: the future covariate support must be covered by the source. When it fails---heavy covariate shift into regions the historical data do not cover---transport is not credible and the estimator's variance degrades. This is the substantive limit of Path~3, and we return to it in the application.
...
\]
where $r(O)$ is the contribution from estimating the density ratio $w$ when the target distribution is itself estimated from a finite sample (and $r \equiv 0$ when the target covariates are treated as fixed/known). For the conditional parallel-trends (difference-in-differences) design, the bracketed augmentation is replaced by the doubly-robust DiD score of \citet{santannaDoublyRobustDifferenceindifferences2020}, conditioned on $\bX$ and reweighted by $w$.

\textit{Orthogonality and rates.} The score $(\ast)$ is Neyman-orthogonal: its Gateaux derivative with respect to each of $\mu_a$, $e$, and $w$ vanishes at the truth. Consequently, estimating these nuisances by cross-fitted nonparametric or machine-learning methods at $L_2$ rate $o_\P(n^{-1/4})$ leaves the resulting product bias at $o_\P(n^{-1/2})$, so the one-step/estimating-equation estimator $\widehat\theta_{p+1}$ is asymptotically linear with influence function $(\ast)$, and $\sqrt{n}(\widehat\theta_{p+1} - \theta_{p+1}) \leadsto N(0, \E[\phi_{\psi_3}^2])$ \citep{chernozhukovDoubleDebiasedMachine2018}. We do \emph{not} require $\widehat\tau(\cdot)$ to be $\sqrt{n}$-consistent; the $\sqrt{n}$ rate is recovered at the level of the scalar functional through orthogonality.
...
\]
which is exactly the efficient influence function of the (augmented inverse-propensity-weighted) average treatment effect. Thus Path~3 nests the no-covariate-shift case as the ordinary doubly-robust ATT estimator.
\end{lemma}

Lemma~\ref{lem:path3-collapse} is a useful correctness check: any implementation of the Path~3 variance should reproduce the standard doubly-robust ATT standard error when the target and source covariate distributions coincide.
