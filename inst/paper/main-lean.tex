...
\medskip
This paper's contribution begins where first-stage identification ends. Given identified $\{\theta_{gt}\}_{g,\, t \leq p}$, when and how do these backward-looking estimands identify the forward-looking FATT, FATU, and FATE? The full identification argument thus has two steps: (a) under the researcher's first-stage design, the relevant first-stage object is identified from observed data; (b) under the extrapolation assumptions of Section~\ref{sec:identification}, the future estimands are identified from it. The three paths in Section~\ref{sec:identification} differ only in what they assume for step (b). We defer all questions of estimation and uncertainty---how the first-stage influence functions propagate to the future estimands---to Section~\ref{sec:inference}.
