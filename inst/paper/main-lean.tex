...
We first require an assumption common to any first-stage strategy that links observed outcomes to potential outcomes.
...
We also require that the covariates used for adjustment be unaffected by the treatment.
...
\end{assumption}
With time-invariant baseline covariates this holds automatically. It is the standard condition that underlies covariate adjustment in any first-stage design (Paths~1--3), and it is invoked a second time by Path~3, which integrates over a \emph{future} covariate distribution: if treatment-affected covariates were used, both the conditional effect and the transport target would be contaminated. When covariates are time-varying \emph{and} affected by treatment, valid adjustment requires g-methods \citep{robinsNewApproachCausal1986}; we assume exogeneity and leave that case to future work.
