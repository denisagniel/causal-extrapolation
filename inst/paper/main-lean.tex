...
We observe $n$ units over $p$ time periods. For unit $i = 1, \ldots, n$ and time $t = 1, \ldots, p$, let $A_{it} \in \{0,1\}$ denote a policy indicator, $Y_{it}$ a scalar outcome of interest, and $\bX_{it}$ a vector of pre-treatment covariates, which may be time-invariant or time-varying. We write $\bX_i$ when covariates are time-invariant or when the relevant period is clear from context. Time-varying covariates matter for Path~3 (Section~\ref{sec:path3}): there the future covariate distribution is the engine of extrapolation. Define $Y_{it}(a)$ as the potential outcome for unit $i$ at time $t$ if the policy were set to $A_{it} = a$, possibly contrary to fact. The following assumption links observed outcomes to potential outcomes.
