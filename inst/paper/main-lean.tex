...
Because identification here rests on parallel trends, we apply the two paths whose forward maps operate directly on the difference-in-differences group-time ATTs:
\begin{enumerate}
...
\end{enumerate}
Path~3 (covariate transport) requires a covariate-conditional effect $\tau(x)$ estimated under the same difference-in-differences identification---a conditional doubly-robust DiD construction whose transport influence function we develop but do not yet validate; we therefore defer the Path~3 column of this application to future work rather than report an unvalidated number.
...
Both paths substantially underpredict the validation-period effects, and the shortfall widens over time. The realized ATT rises from roughly $1.0$ in 2016 to about $2.0$--$2.3$ by 2020--2022 (peaking at $2.25$ in 2021), an acceleration neither path anticipates from pre-2015 data.
\begin{itemize}
...
\end{itemize}
Path~2 dominates Path~1---modeling temporal dynamics helps---but neither is close: the effect grew faster after 2015 than any model fit to 1981--2015 could have projected.
...
What the framework cannot manufacture is information about the future that the past does not contain. Both paths underpredicted, with gaps widening over time, because the SYG effect accelerated after 2015 faster than any model fit to 1981--2015 would project. Plausible drivers include a genuine post-2015 regime change (cultural shifts, firearm-purchasing surges, the 2020 disruption) not reflected in the training period. This is not a failure of inference but the intrinsic limit of extrapolation: valid intervals quantify \emph{estimation} uncertainty, not the risk that the invariance assumption itself breaks.

\paragraph{Lesson}

Reporting this shortfall plainly is the point. Because we held out 2016--2022, we can see when the assumptions break; in real policy settings such validation is rarely available, which is precisely why the forward-looking estimand and its assumptions must be stated explicitly rather than buried in a backward-looking average. The honest finding---temporal modeling helps but does not rescue extrapolation across a regime change---is a stronger and more useful result than an artificially good one.

Additional application details (data sources, covariates, and the deferred Path~3 construction) are in Appendix~E.
