...
| 6 | EIF section (old `main.tex:471-637`), retitled and re-labeled (`\label{sec:eif}\label{sec:inference}`, both kept — `sec:eif` because dozens of appendix proof cross-references depend on it). Contains Path 3's two-regime EIF material, relocated from §5 as a third `\paragraph` alongside Path 1's and Path 2's, argued once across all three paths per the outline's stated inference-asymmetry point. **Now also contains genuinely new content (added later the same day, "Item 2" — see `session_notes/2026-09-25.md`'s 15:00 and later entries): a new subsection `sec:md-aggregation`** answering "how should the aggregation across possibly-correlated cells be formed" for Paths 1/2, via `Proposition~\ref{prop:md-aggregation}` (GMM/minimum-distance-efficient combination) plus two remarks stating precisely what efficiency it does and does not deliver. This is not a restructuring of existing prose — it is new theory, written up from the R package's `combine_cell_scores()` design (Oracle-consulted for both the R implementation and the paper integration). | **drafted 2026-09-25** (`unblessed`, `main.tex:424-586` for the restructured EIF material; `main.tex:566-630` for the new `sec:md-aggregation` content). Content (EIF formulas, asymptotic-normality proposition, Path 3's two-regime derivation) verbatim from its prior location; only the section title, the addition of `sec:inference`, the intro paragraph (rewritten to state the propagate-vs-not-propagate asymmetry as the section's organizing point), and the physical relocation of the Path-3 paragraph changed by the restructuring pass. `sec:md-aggregation` is new: new Assumption `RC11` (appendix), new appendix proof `app:prop-md`, and a one-sentence forward pointer added to the existing Path-1 EIF paragraph. One internal appendix cross-reference (`app:eif3`'s "as displayed in Section~\ref{sec:path3}") was updated to `\ref{sec:eif-derivation}` to follow the earlier move; the `RC1--RC10` range citation was updated to `RC1--RC11` at both its body and appendix occurrences (closing, rather than repeating, the exact "restatement lags the fix" failure the 2026-09-24 session hit once already). |
