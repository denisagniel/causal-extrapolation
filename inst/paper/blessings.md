...
| `main.tex` §6, new `\subsection{Aggregating correlated cells}` (`main.tex:566-630`, `\label{sec:md-aggregation}`) — **Item 2, genuinely new content, not restructured** | 2026-09-25, later same-day pass | `unblessed` | New `Proposition~\ref{prop:md-aggregation}` (Efficient aggregation of correlated cells) + two remarks (`rem:md-efficiency`, `rem:md-omega`) stating the GMM/minimum-distance-efficiency claim precisely — not the semiparametric bound. Design consulted with Oracle twice: once for the R package's `combine_cell_scores()` (already merged), once for this paper integration specifically. New Assumption `RC11` (appendix, `main.tex:951`) and new appendix proof `\subsection{Proof of the Efficient-Aggregation Proposition}` (`app:prop-md`, `main.tex:1584`). One-sentence forward pointer added to the existing Path-1 EIF paragraph (`main.tex:432`); one-sentence efficiency-claim correction to the existing `app:eif1` step-4 text (`main.tex:1096`) to remove a now-contradicted overclaim ("attains the semiparametric efficiency bound... when the supplied [ingredients] are themselves efficient" — true necessary condition, false as a sufficient one, per `rem:md-efficiency`). `RC1--RC10` range citations widened to `RC1--RC11` at both occurrences (body `prop:asymp` statement and its appendix restatement), updated together. |
