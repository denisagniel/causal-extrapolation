...
| Proposition (Efficient aggregation of correlated cells) (added 2026-09-25, "Item 2") | `main.tex:593` (§6, new `\ref{sec:md-aggregation}` subsection) | For a linear restriction $\btheta=X\bgamma$ overidentified by $J$ correlated cells with stacked covariance $\bSigma$, the minimum-distance/optimal-GMM combination $\blambda^*=\bSigma^{-1}XH^{-1}\bx_{p+1}$ is asymptotically linear (i), unbiased for $\theta_{p+1}$ (ii), variance-minimizing within that linear class (iii), and reduces to inverse-variance-then-$\omega$-weighting when cells are independent (iv) | `RC1`-`RC3`, `RC6`, `RC11` (new, added same pass) | §6 body statement (`main.tex:566-598`); not separately claimed in Abstract/Discussion — this is an inference-machinery refinement of the already-claimed C4/S6 EIF-propagation story, not a new headline claim | methodological | `RC1`-`RC3`, `RC6`, `RC11`; full column rank of $X$ is `RC5`(ii)/`cor:linear-rank`, not new |
| Proposition (Asymptotic distribution of $\widehat\theta_{p+1}$) | `main.tex:634` (line shifted by the 2026-09-25 Item-2 insertion above it; label unchanged) | $\sqrt n\{\widehat\theta_{p+1}-\theta_{p+1}\}\leadsto N(0,\sigma^2)$, $\sigma^2=\E(\phi_i^2)$, for whichever path/regime is selected | RC1-RC11 (the applicable subset per path/regime — see `notation.md` §2a's range-citation note; range widened from RC1-RC10 to RC1-RC11 in the same 2026-09-25 pass that added RC11, both the body statement and the appendix restatement updated together) | Abstract ("standard errors and confidence intervals propagate correctly", `main.tex:44`); §6 (`main.tex:634-643`); Discussion | — | S6 — *"All three yield asymptotically normal estimators whose variance is the mean square of the corresponding influence function."* |
