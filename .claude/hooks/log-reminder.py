#!/usr/bin/env python3
"""
Session Log Reminder Hook for AI coding agents

A Stop hook that tracks how many responses have passed since the session
log was last updated. After a threshold, it blocks the agent from stopping
...
def get_project_dir():
    ...
    # If stop_hook_active, the agent is already continuing from a previous
...
def main():
    ...
    # Case 1: No session log exists at all — remind once, then let the agent work
    if latest_log is None:
        ...
        # Already reminded — let the agent proceed (it will create the log)
        sys.exit(0)
...
if __name__ == "__main__":
    try:
        ...
        # Fail open — never block the agent due to a hook bug
        sys.exit(0)
