...
\end{figure}
When the slope in event time is non-zero, the two curves in Figure~\ref{fig:backward-vs-fatt} separate; interpreting the backward-looking ATT as the policy-relevant effect is then misleading. This motivates the need for either time-homogeneity assumptions (Path~1) or explicit extrapolation (Path~2).
...
Under correct specification, the EIF-based variance and Wald intervals from Section~5 should achieve nominal coverage.
...
Taken together, the simulations reinforce the paper's message: the target of estimation (FATT vs backward-looking ATT) matters; Path~1 and Path~2 each have their place depending on whether the researcher is willing to assume time homogeneity or to model dynamics; and EIF-based inference is well-calibrated under correct specification.
...
We enumerate the technical assumptions needed for the identification and asymptotic results in Sections 4--5.
...
\begin{remark}
The structural assumption that $f(g,t;\bgamma)$ continues to hold at $t = p+1$ is crucial and untestable. It reflects a belief that the dynamic process generating $\theta_{gt}$ for $t \leq p$ persists into the future. Misspecification of $f$ leads to bias (see Simulations in Section 6).
\end{remark}
...
We restate and prove Proposition 3 from Section 5.1.
