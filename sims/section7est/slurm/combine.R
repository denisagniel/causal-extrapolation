...
# against the expected task count, DE-DUPLICATES by (unit, arm) (keeping the
...
#
# NOTE (section7est-specific): run_one() emits FOUR rows per unit (one per
# estimator arm: path1, path2, path3_oracle, path3_grf). The de-dup therefore
# keys on (unit, arm) rather than `unit` alone -- keying on `unit` would collapse
# each unit to a single arm and silently discard 75% of the results.
#
...
#   * De-duplicates by (unit, arm) (duplicate task files -- e.g. from a re-run --
#     otherwise inflate N).
#   * Asserts every unit in 1..TOTAL_UNITS appears (unless partial).
...
# Read newest-file-LAST so that on a duplicate (unit, arm) the newest row wins
# the later de-dup (which keeps the last occurrence).
...
# --- De-duplicate by (unit, arm) (keep newest = last occurrence) -------------
# section7est emits FOUR rows per unit (one per estimator arm), so the canonical
# de-dup-by-`unit` would collapse each unit to a single arm. Key on (unit, arm)
# so a re-run's duplicate (unit, arm) pairs are removed without dropping arms.
n_raw <- nrow(result)
dup <- duplicated(result[, c("unit", "arm")], fromLast = TRUE)
if (any(dup)) {
  cat(sprintf("De-dup: dropping %d duplicate (unit, arm) row(s) (kept newest).\n", sum(dup)))
...
# --- Coverage assertion: every unit present (catches mapping bugs) -----------
if (!opt$allow_partial) {
